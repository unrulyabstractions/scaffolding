...
def _import_safe(name: str, package: str) -> Any | None:
    ...
    try:
        ...
        # Swallowing keeps optional-dependency modules (e.g. mlx backends) from
        # breaking package import, but it also hides real bugs. Set
        # AUTO_EXPORT_DEBUG=1 to surface what gets skipped.
        if os.environ.get("AUTO_EXPORT_DEBUG"):
            print(f"[auto_export] swallowed in {package}.{name}: {e}", file=sys.stderr)
        return None
